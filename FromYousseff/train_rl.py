...
from config import LEARNING_RATE, PPO_TOTAL_STEPS
from train_common import callbacks, get_device, make_training_env, parse_args, setup_dirs


def main():
    args = parse_args(PPO_TOTAL_STEPS, default_envs=4)
    device = args.device if args.device != "auto" else get_device(prefer_cuda=False)
    ...
    env.close()
    print("Saved models/qube_ppo_final.zip")
